...
describe("BlueBubbles webhook monitor", () => {
  ...
  function createHangingWebhookRequest(url = "/bluebubbles-webhook?password=test-password") {
    const req = new EventEmitter() as IncomingMessage;
    const destroy: HangingWebhookDestroy = vi.fn<IncomingMessage["destroy"]>(
    ...
    req.url = url;
    req.headers = {};
    req.destroy = destroy as IncomingMessage["destroy"];
    setRequestRemoteAddress(req, "127.0.0.1");
    return req as IncomingMessage & { destroy: HangingWebhookDestroy };
  }
